...
````
- Так же можно проверить определенные статусы, например чтобы проверить активен ли (работает ли) модуль в данный момент можно использовать команду `is-active` 
 `systemctl is-active имя_службы.service` Это вернет текущий статус модуля, который обычно `active` или `inactive`. 
...
## Отображение файла юнита
Чтобы отобразить файл юнита, который система `systemd` загрузила в систему, можно использовать команду `cat`
...
тобы увидеть свойства более низкого уровня модуля, можно использовать команду `show`. При этом будет выведен список свойств, заданных для указанного модуля с помощью формата `key=value`:
...
. . .
```
Если вы хотите отобразить одно свойство, можно передать флаг `-p` с именем свойства.
`systemctl show sshd.service -p Conflicts`\
```
OutputConflicts=shutdown.target
```
## Маскировка и снятие маскировки юнитов

`systemd` также имеет возможность отметить модуль как _полностью_ незапускаемый, автоматически или вручную
...
Команда `edit` по умолчанию откроет фрагмент файла модуля для интересующего юнита. `sudo systemctl edit nginx.service` 
...
Если вы хотите редактировать весь файл модуля, а не создавать фрагмент, можно передать флаг `--full`   `sudo systemctl edit --full nginx.service` 
...
Чтобы удалить какие-либо сделанные добавления, удалите либо каталог конфигурации модуля `.d` или модифицированный служебный файл из `/etc/systemd/system`.
Например, для удаления фрагмента можно ввести следующее:
...
## Настройка состояния системы с помощью таргетов
...
 Они используются в качестве справки, когда доступны определенные функции, позволяя вам указывать желаемое состояние вместо необходимости использования отдельных модулей для получения этого состояния.
...
### Получение и найстройка таргета по умолчанию
