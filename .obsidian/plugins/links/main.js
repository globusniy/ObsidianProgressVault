...
var ObsidianLinksSettingTab = class extends import_obsidian6.PluginSettingTab {
  ...
  display() {
    ...
    if (appendMdExtensionDescription) {
      ...
      const settingCopyLinkToHeadingContextMenu = new import_obsidian6.Setting(containerEl).setName("Copy link to heading").setDesc("").addToggle((toggle) => {
      ...
      const toggleCopyLinkToHeadingContextMenuSetting = (enabled) => {
        if (enabled) {
      ...
      const settingCopyLinkToBlockContextMenu = new import_obsidian6.Setting(containerEl).setName("Copy link to block").setDesc("").addToggle((toggle) => {
        toggle.setValue(this.plugin.settings.contexMenu.copyLinkToBlockToClipboard).onChange(async (value) => {
          this.plugin.settings.contexMenu.copyLinkToBlockToClipboard = value;
          await this.plugin.saveSettings();
        });
      });
      const toggleCopyLinkToBlockContextMenuSetting = (enabled) => {
        if (enabled) {
      ...
      new import_obsidian6.Setting(containerEl).setName("Copy link to block").setDesc("Copy link to a block to the clipboard").setClass("setting-item-copy-link-to-block").addToggle((toggle) => {
        toggle.setValue(this.plugin.settings.ffCopyLinkToBlock).onChange(async (value) => {
...
// commands/CopyLinkToBlockToClipboardCommand.ts
var CopyLinkToBlockToClipboardCommand = class extends CommandBase {
  constructor(obsidianProxy, isPresentInContextMenu = () => true, isEnabled = () => true) {
    super(isPresentInContextMenu, isEnabled);
